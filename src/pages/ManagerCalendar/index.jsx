...
function ManagerCalendar() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const { data } = await api.auth.getAllTusks()
      setData(data)
    }
    fetchData()
  }, [])
  ...
  function getListData(value) {
    let tusks = []
    data.forEach((item) => {
      if (item.object.worker_id.id === auth.user.id && !item.completeness) {
        if (
          ...
          {dataModal.tasks.length > 0 ? (
            dataModal.tasks.map((item) => (
              <div
                ...
                key={item.id}
                onClick={() => {
                  navigate(`/manager-object-task/${item.id}`)
                }}
